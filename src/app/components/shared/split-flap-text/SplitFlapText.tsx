...
// El CSS NO se importa aqui (a diferencia de la copia original de React
// Bits): un `import './SplitFlapText.css'` dentro de un .tsx termina en un
// chunk que el bundler de Angular (esbuild) puede marcar como LAZY -- se
// comprobo en build real: el archivo llegaba a dist/.../main.css pero listado
// bajo "Lazy chunk files", así que en produccion nunca se pedia (el header es
// parte del shell EAGER) y el componente se veia sin ningun estilo (cada
// caracter duplicado: sin position:absolute/overflow:hidden, las dos mitades
// del tile se pintan una tras otra en vez de superpuestas). El mismo CSS se
// referencia ahora desde `styleUrls` en split-flap-text.component.ts, que es
// el mecanismo de Angular para estilos de componente y no tiene esa
// ambiguedad de chunking.
...
const SplitFlapText = ({
  ...
  // number, no ReturnType<typeof setTimeout>: este proyecto trae @types/node
  // (para el tooling de build) y ese alias termina resolviendo al overload
  // de Node (NodeJS.Timeout) en vez del de DOM -- que es el que realmente
  // devuelve window.setTimeout, siempre usado aqui explicitamente.
